...
async function main() {
  ...
  ok("colunas introspectadas", empty.columns.map((c) => c.name).join(",") === "id,nome,meta", empty.columns.map((c) => c.name));
  ...
  ok(
    ...
    typeof inserted.meta === "string" && JSON.stringify(JSON.parse(inserted.meta as string)) === JSON.stringify({ tier: "gold", n: 1 }),
    inserted.meta,
  );

  await insertRow(conn, { schema: "public", table: "clientes", values: { nome: "Linus", meta: null } });
  const two = await browseRows(conn, { schema: "public", table: "clientes", orderBy: "id", orderDir: "asc" });
  ...
  ok("filtro ILIKE", filtered.totalCount === 1 && String(filtered.rows[0]?.nome) === "Ada", filtered.totalCount);
  const paged = await browseRows(conn, { schema: "public", table: "clientes", limit: 1, offset: 1, orderBy: "id", orderDir: "asc" });
  ok("paginação limit/offset", paged.rows.length === 1 && String(paged.rows[0]?.nome) === "Linus" && paged.totalCount === 2);
  ...
  ok(
    ...
    typeof jsonbEdit.meta === "string" && JSON.stringify(JSON.parse(jsonbEdit.meta as string)) === JSON.stringify({ tier: "platinum", n: 2 }),
    jsonbEdit.meta,
  );
  ...
  ok("coluna ativo adicionada com default", withCol.columns.some((c) => c.name === "ativo"), withCol.columns.map((c) => c.name));
  ok("default backfilled", withCol.rows.every((r) => r.ativo === true), withCol.rows.map((r) => r.ativo));

  await runDdl(conn, { op: "rename_column", schema: "public", table: "clientes", column: "ativo", to: "habilitado" });
  const renamedCol = await browseRows(conn, { schema: "public", table: "clientes" });
  ok(
    "coluna renomeada ativo→habilitado",
    renamedCol.columns.some((c) => c.name === "habilitado") && !renamedCol.columns.some((c) => c.name === "ativo"),
    renamedCol.columns.map((c) => c.name),
  );

  console.log("\n== DDL: create_index + drop_index ==");
  await runDdl(conn, { op: "create_index", schema: "public", table: "clientes", columns: ["nome"], unique: false, name: "idx_clientes_nome" });
  const idxCount1 = await runQuery(conn, "SELECT count(*)::int AS n FROM pg_indexes WHERE indexname = 'idx_clientes_nome'");
  ...
  const idxCount2 = await runQuery(conn, "SELECT count(*)::int AS n FROM pg_indexes WHERE indexname = 'idx_clientes_nome'");
  ...
  await expectThrow("tabela antiga some", () => browseRows(conn, { schema: "public", table: "clientes" }), "table_not_found");
  ...
  ok("runQuery retorna fields+rows", q.fields.includes("nome") && q.rowCount === 1, { fields: q.fields, rowCount: q.rowCount });

  console.log("\n== Segurança: identificadores & colunas inválidas ==");
  await expectThrow("ident inválido em tabela", () => browseRows(conn, { schema: "public", table: 'x"; DROP TABLE customers;--' }));
  await expectThrow("coluna inexistente em orderBy", () => browseRows(conn, { schema: "public", table: "customers", orderBy: "naoexiste" }), "column_not_found");
  await expectThrow("insert em coluna inexistente", () => insertRow(conn, { schema: "public", table: "customers", values: { hacker: 1 } }), "column_not_found");
  await expectThrow("update sem pk", () => updateRow(conn, { schema: "public", table: "customers", pk: {}, set: { nome: "x" } }), "missing_pk");
  await expectThrow("tipo de coluna fora do allowlist", () =>
    runDdl(conn, { op: "add_column", schema: "public", table: "customers", column: { name: "bad", type: "text); DROP TABLE customers;--", nullable: true, primaryKey: false, default: null } }),
  );
  await expectThrow("default com comentário SQL rejeitado", () =>
    runDdl(conn, { op: "add_column", schema: "public", table: "customers", column: { name: "x", type: "text", nullable: true, primaryKey: false, default: "'a'-- comment" } }),
  );

  console.log("\n== DDL: drop_table (limpeza) ==");
  await runDdl(conn, { op: "drop_table", schema: "public", table: "customers" });
  await expectThrow("tabela dropada", () => browseRows(conn, { schema: "public", table: "customers" }), "table_not_found");

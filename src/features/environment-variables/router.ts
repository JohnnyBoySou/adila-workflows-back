...
export const environmentVariablesRouter = new Elysia({
  ...
  .get(
    ...
    async ({ organizationId, params, query, status, role }) => {
      ...
      return environmentVariablesController.list(organizationId, params.id, reveal);
    },
  ...
  .post(
    ...
    async ({ organizationId, user, params, body, status, request }) => {
      ...
      const result = await environmentVariablesController.create(organizationId, params.id, body);
